...
function FootballWipeText({
  ...
  const ballIndex = transitionKey.length % sportsBalls.length
  const selectedBall = sportsBalls[ballIndex]
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <motion.div
            ...
            transition={{ duration: 0.6, ease: 'easeInOut' }}
          >
            <span className="text-4xl drop-shadow-lg">{selectedBall}</span>
          </motion.div>
        </motion.div>
      </AnimatePresence>
    </div>
...
export function LandingPage() {
  ...
  const categories = [
    ...
    {
      id: 4,
      name: t('fitnessTrainer'),
      icon: '💪',
      nameAr: 'مدربين لياقة بدنية',
      nameEn: 'Fitness Trainers',
    },
    {
      id: 5,
      name: t('naturalField'),
      icon: '⚽',
      nameAr: 'معالجين طبيعيين',
      nameEn: 'Natural Field',
    },
    {
      id: 6,
      name: t('nutritionist'),
      icon: '🥗',
      nameAr: 'اخصائيين تغذية',
      nameEn: 'Nutritionists',
    },
    {
      id: 7,
      name: t('smartSolution'),
      icon: '📊',
      nameAr: 'محللين اداء',
        ...
        <section className="py-12 sm:py-16 px-4 sm:px-6 bg-gradient-to-br from-purple-50 via-white to-blue-50">
          <div className="max-w-7xl mx-auto">
            ...
            >
              ...
              <p className="text-gray-600 text-base sm:text-lg max-w-2xl mx-auto">
                {language === 'ar'
                  ? 'تعرف على أفضل اللاعبين المحترفين ذوي التقييمات العالية'
