...
import type { Photo, Video as VideoType } from '@/types/player'
...
const GalleryPage = () => {
  ...
  const handleAddPhoto = async () => {
    ...
    try {
      setAdding(true)
      await playerService.addPhoto(selectedFile, photoCaption, photoCaptionAr)
  ...
  const handleRemovePhoto = async (photoId: string) => {
    ...
    try {
      await playerService.removePhoto(photoId)
      // Refresh gallery
      await fetchGalleryData()
    } catch (err: any) {
  ...
  const handleAddVideo = async () => {
    ...
    try {
      setAddingVideo(true)
      const updatedVideos = await playerService.addVideo({
        url: videoUrl,
        title: videoTitle,
  ...
  const handleRemoveVideo = async (videoId: string) => {
    ...
    try {
      const updatedVideos = await playerService.removeVideo(videoId)
      setVideos(updatedVideos)
    } catch (err) {
