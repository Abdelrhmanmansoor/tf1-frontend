...
import type { Photo, Video as VideoType } from '@/types/club'

const ClubGalleryPage = () => {
  ...
  const fetchGalleryData = async () => {
    try {
      ...
      setPhotos(profile.facilityPhotos || [])
      setVideos(profile.facilityVideos || [])
    } catch (err) {
  ...
  const handleAddPhoto = async () => {
    ...
    try {
      setAdding(true)
      await clubService.uploadFacilityPhoto(
        selectedFile,
        photoCaption,
        photoCaptionAr
      )
  ...
  const handleRemovePhoto = async (photoId: string) => {
    ...
    try {
      await clubService.removeFacilityPhoto(photoId)
      // Refresh gallery
      await fetchGalleryData()
    } catch (err: any) {
  ...
  const handleAddVideo = async () => {
    ...
    try {
      setAddingVideo(true)
      await clubService.addFacilityVideo({
        url: videoUrl,
        thumbnail: videoThumbnail,
        title: videoTitle,
        titleAr: videoTitleAr,
      })
  ...
  const handleRemoveVideo = async (videoId: string) => {
    ...
    try {
      await clubService.removeFacilityVideo(videoId)
      // Refresh gallery
      await fetchGalleryData()
    } catch (err) {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-2xl font-bold text-gray-900">
                {language === 'ar' ? 'معرض المنشأة' : 'Facility Gallery'}
              </h1>
            </div>
            <Button
              onClick={() =>
            ...
            {photos.length === 0 && (
              <div className="col-span-full flex flex-col items-center justify-center py-20 text-gray-500">
                ...
                <p className="text-sm mt-2">
                  {language === 'ar'
                    ? 'أضف صور منشأتك الأولى!'
                    : 'Add your first facility photos!'}
                </p>
                <Button
